	...
	def run(self): 
		...
		while True:
			try:
				...
				sys.stdout.write(f"\rThread {self.counter}: Requisição enviada de {str(proxy[0]+':'+proxy[1])} ")
				sys.stdout.flush()
				try: 
		...
		while True:
			try:
				...
				try:
					...
					sys.stdout.write(f"\rThread {self.counter}: Sock down. Tentando novamente. ")
					sys.stdout.flush()
					s.close() 
	...
	def run(self): 
		...
		while True: 
			try: 
				...
				s.send(p)
				# Atualiza a mesma linha no terminal
				sys.stdout.write(f"\rThread {self.counter}: Requisição Enviada! ")
				sys.stdout.flush()
				try: 
					for y in range(multiple): 
						s.send(str.encode(p)) 
